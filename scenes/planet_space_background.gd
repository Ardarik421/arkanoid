...
static func _draw_space(c: CanvasItem, progress: float) -> void:
	...
	var bottom: Color = Color(0.004, 0.025, 0.055).lerp(Color(0.006, 0.018, 0.045), progress)
...
static func _draw_nebula(c: CanvasItem, progress: float, time: float) -> void:
	...
	for j in range(centers.size()):
		...
		for i in range(7, 0, -1):
			...
			var col: Color = Color(0.06, 0.48, 0.78, a) if j % 2 == 0 else Color(0.32, 0.14, 0.62, a * 0.75)
			c.draw_circle(p, r, col)
	for i in range(9):
		var y: float = 80.0 + float(i) * 47.0
		var x1: float = 360.0 + sin(time * 0.025 + i) * 25.0
		c.draw_line(Vector2(x1, y), Vector2(1010, y + 160), Color(0.10, 0.55, 0.80, 0.018), 22.0, true)
...
static func _draw_stars(c: CanvasItem, progress: float, time: float) -> void:
	for i in range(115):
		...
		c.draw_circle(Vector2(x, y), radius, Color(0.68, 0.88, 1.0, alpha))
		if i % 19 == 0:
			var flare: float = 5.0 + pulse * 7.0
			c.draw_line(Vector2(x - flare, y), Vector2(x + flare, y), Color(0.55, 0.82, 1.0, 0.12 + pulse * 0.18), 1.0, true)
...
static func _draw_sun(c: CanvasItem, progress: float, time: float) -> void:
	...
	for i in range(16):
		...
		c.draw_line(center + direction * 9.0, center + direction * length, Color(1.0, 0.70, 0.35, 0.18 * pulse), 1.2, true)
	c.draw_circle(center, 9.0 * pulse, Color(1.0, 0.90, 0.67, 0.82))
...
static func _draw_moon(c: CanvasItem, progress: float, time: float) -> void:
	...
	c.draw_arc(center, radius, -1.42, 1.72, 64, Color(0.48, 0.78, 1.0, 0.62), 2.2, true)
...
static func _draw_asteroid(c: CanvasItem, center: Vector2, radius: float, phase: float, depth: float) -> void:
	...
	c.draw_polyline(outline, Color(0.25, 0.40, 0.52, 0.20 + depth * 0.22), 1.0, true)
...
static func _draw_planet(c: CanvasItem, progress: float, time: float) -> void:
	...
	c.draw_circle(center, radius, Color(0.008, 0.055, 0.105, 1.0))
	for i in range(8):
		...
		c.draw_line(Vector2(center.x - half_width * 0.78 + shift, band_y), Vector2(center.x + half_width * 0.72 + shift, band_y - 12.0), Color(0.28, 0.72, 0.88, 0.055), 18.0, true)
	for i in range(15):
		var a: float = float(i) * 2.37
		var rr: float = radius * (0.20 + float((i * 17) % 65) / 100.0)
		var p: Vector2 = center + Vector2(cos(a) * rr, sin(a) * rr * 0.72)
		c.draw_circle(p, 18.0 + float(i % 4) * 8.0, Color(0.04, 0.22, 0.25, 0.12))
	c.draw_circle(center + Vector2(-radius * 0.48, radius * 0.28), radius * 0.88, Color(0.001, 0.006, 0.014, 0.62))
	c.draw_arc(center, radius, -1.58, 1.05, 128, Color(0.36, 0.86, 1.0, 0.82), 4.0, true)
	c.draw_arc(center, radius + 8.0, -1.64, 1.12, 128, Color(0.16, 0.66, 1.0, 0.28), 10.0, true)
	c.draw_arc(center, radius + 23.0, -1.68, 1.15, 128, Color(0.12, 0.48, 0.92, 0.09), 18.0, true)
...
static func _draw_dust(c: CanvasItem, progress: float, time: float) -> void:
	for i in range(38):
		...
		c.draw_circle(Vector2(x + drift, base_y - rise), 0.6 + float(i % 3) * 0.4, Color(0.42, 0.78, 1.0, 0.06 + float(i % 4) * 0.018))
