...
static func _draw_stars(canvas: Node2D, progress: float, time: float):
	...
	for i in range(5+int(progress*4.0)):
		...
		if brightness > 0.06:
			var a = brightness*(0.10+progress*0.07)
...
static func _draw_satellites(canvas: Node2D, progress: float, time: float):
	var positions = [Vector2(790,650),Vector2(145,760),Vector2(725,885)]
	var visible = 1+int(progress*2.8)
	for i in range(min(visible,positions.size())):
		var p = positions[i]+Vector2(sin(time*0.055+i)*12.0,cos(time*0.043+i)*7.0)
		var s = 0.72+float(i)*0.10
		canvas.draw_rect(Rect2(p-Vector2(11,4)*s,Vector2(22,8)*s),Color(0.16,0.20,0.26,0.44))
		canvas.draw_line(p+Vector2(-12,0)*s,p+Vector2(-27,0)*s,Color(0.28,0.38,0.48,0.30),1.2,true)
		canvas.draw_line(p+Vector2(12,0)*s,p+Vector2(27,0)*s,Color(0.28,0.38,0.48,0.30),1.2,true)
		canvas.draw_rect(Rect2(p+Vector2(-35,-7)*s,Vector2(10,14)*s),Color(0.06,0.17,0.30,0.32))
		canvas.draw_rect(Rect2(p+Vector2(25,-7)*s,Vector2(10,14)*s),Color(0.06,0.17,0.30,0.32))
		canvas.draw_circle(p,1.5*s,Color(0.48,0.72,0.92,0.30))
